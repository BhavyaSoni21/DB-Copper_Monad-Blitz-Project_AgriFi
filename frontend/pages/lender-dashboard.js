  ...
  async function fetchFundedLoans() {
    ...
    for (let i = 0; i < 20; i++) {
      try {
        ...
        if (loan.lender.toLowerCase() === myAddress.toLowerCase()) arr.push({ ...loan, id: i });
      } catch {}
    }
    setLoans(arr);
    setLoading(false);
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-green-50">
      <div className="bg-white p-8 rounded shadow w-full max-w-md">
        <h2 className="text-2xl font-bold mb-4 text-green-700">Lender Dashboard</h2>
        <div className="mb-4 text-xs text-gray-500">Network: {network}</div>
        {loading && <div className="mb-4 text-green-600">Loading...</div>}
        {loans.length === 0 && !loading && <div>No funded loans found.</div>}
        {loans.map(loan => (
